...
template <typename T>
T Timer::getDataForRandom(std::vector<T> &dataset)
{
...
template <typename T>
unsigned long long Timer::ListDelete()
{
    ...
    for (int i = 0; i < datasetGenerationCount; i++)
    {
        ...
        for (int i = 0; i < loopCount; i++)
        {
            ...
            while (!dataset.empty())
            {
                auto value = getDataForRandom(dataset);
...
void Timer::run()
{
    ...
    for (auto size : datasetSizesToTest)
    {
        ...
        logger.logToFile(ListDelete<datatype>());
        logger.logToFile(GenericRemove<Array, datatype>(array_push_back));
