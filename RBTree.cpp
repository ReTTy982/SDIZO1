...
template <typename T>
inline auto RBTree<T>::get_parent_slot(const RBTree<T>::Node *node) -> RBTree<T>::Node *&
{
    if (node == root)
    {
        return root;
...
template <typename T>
inline auto RBTree<T>::get_uncle(const RBTree<T>::Node *node) -> RBTree<T>::Node *&
{
    if (node->parent->left_child == node)
    {
        return node->parent->right_child;
...
void RBTree<T>::rotateR(RBTree<T>::Node *node)
{
    ...
    *child_from = parent;
    ...
    get_parent_slot(parent) = node; // dowiadujemy sie jakim dzieckiem byl wezel wokol ktorego rotujemy
...
template <typename T>
bool RBTree<T>::bst_delete(const T &value)
{
}
